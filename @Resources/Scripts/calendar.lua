...
function FillHeader()
	...
	SKIN:Bang('!SetOption','CurrentMonthIcon','Text',tonumber(os.date('%m',os.time())))
	SKIN:Bang('!SetOption','CurrentMonthText','Text',os.date('%B',os.time()))
	-- Next month header
	if tonumber(os.date('%m',RemoveTime(FindLastSunday(os.time()))+(7*WeekCount-1)*86400)) ~= tonumber(os.date('%m')) then		-- Need to draw next month header if the last displayed date is not in this month
		...
		SKIN:Bang('!SetOption','NextMonthDivider','Y','#Y'..(WeeksAway+1)..'#+0.04*#GlobalHeight#')
		-- Fill text
		SKIN:Bang('!SetOption','NextMonthIcon','Text',tonumber(os.date('%m',os.time{year=os.date('%Y'),month=os.date('%m')+1,day=1})))
		SKIN:Bang('!SetOption','NextMonthText','Text',os.date('%B',os.time{year=os.date('%Y'),month=os.date('%m')+1,day=1}))
		SKIN:Bang('!ShowMeterGroup', 'NextMonthHeader')
		SKIN:Bang('!RedrawGroup', 'NextMonthHeader')
	else
		SKIN:Bang('!HideMeterGroup', 'NextMonthHeader')
		SKIN:Bang('!RedrawGroup', 'NextMonthHeader')
	end
end
	...
	for i=1,6 do			-- iterate rows
		for j=0,6 do		-- iterate columns
			...
			if HideDate(time_counter) == false then
				...
				-- Style of the string
				if time_counter == RemoveTime(os.time()) then		-- is it today?
				...
				SKIN:Bang('!SetOption', 'D'..i..j, 'X', SKIN:ReplaceVariables('#X'..j..'#'))
				if (tonumber(os.date('%Y',time_counter)) - tonumber(os.date('%Y')))*12 + tonumber(os.date('%m',time_counter)) > tonumber(os.date('%m')) then
					if tonumber(os.date('%w',os.time{year=os.date('%Y'),month=os.date('%m')+1,day=1}))==0 then	-- Remove extra padding if the next month starts on a Sunday
						SKIN:Bang('!SetOption', 'D'..i..j, 'Y', SKIN:ReplaceVariables('#Y'..i..'#+#NewMonthPadding#-#Ydelta#'))
					else
						SKIN:Bang('!SetOption', 'D'..i..j, 'Y', SKIN:ReplaceVariables('#Y'..i..'#+#NewMonthPadding#'))
					end
				else
				...
				if has_value(events,'daycode',time_counter) then
					...
					SKIN:Bang('!SetOption', 'EventCircleD'..i..j, 'LeftMouseUpAction', 'https://calendar.google.com/calendar/r/day/'..tonumber(os.date('%Y', time_counter))..'/'..tonumber(os.date('%m', time_counter))..'/'..tonumber(os.date('%d', time_counter)))
					-- Position of the circle
					SKIN:Bang('!SetOption', 'EventCircleD'..i..j, 'X', SKIN:ReplaceVariables('#X'..j..'#'))
